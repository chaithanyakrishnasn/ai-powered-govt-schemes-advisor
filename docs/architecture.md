...
The application is deployed across three distinct, specialized cloud platforms to optimize for performance, cost, and developer experience.

-   **Frontend (Vercel):** The Next.js 15 application is hosted on Vercel, leveraging its global Edge Network for fast static asset delivery and seamless support for App Router features, Server-Side Rendering (SSR), and Server-Sent Events (SSE).
-   **Backend (Render):** The FastAPI Python application is containerized using Docker and deployed as a Web Service on Render. Render handles the asynchronous workload of the matching engine and manages the long-lived SSE connections required by the Chat Interface.
-   **Database (Supabase):** The PostgreSQL 16 database is hosted on Supabase. We chose Supabase primarily because its free tier natively supports the `pgvector` extension, which is a hard requirement for our semantic search capabilities. It provides robust connection pooling (via `asyncpg`) to handle the concurrent demands of the FastAPI backend.
